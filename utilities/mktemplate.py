...
def create_types(lis: typing.Optional[list[str]]) -> list[type]:
    if lis is None:
        return []
    return [get_module_value(x)[1] for x in lis]


def create_type_messages(
    d: typing.Optional[dict[str, str]]
) -> dict[type, str]:
    if d is None:
        return {}
...
class Params:
    ...
    def __init__(
        self,
        original: dict[str, typing.Any],
        parent: typing.Optional["Params"] = None,
    ):
        self.parent = parent
...
class Template:
    ...
    def per_field(
        ...
        if params.is_field_excluded(fieldname) or params.is_type_excluded(
            fieldtype
        ):
            return ""

        msg = params.get_field_message(fieldname) + params.get_type_message(
            fieldtype
        )
        ...
        if params.is_field_opaque(fieldname) or params.is_type_opaque(
            fieldtype
        ):
            dec: str
            if orig == "list":
        ...
        assert issubclass(
            ty, self.root
        ), f"{ty} is not a subclass of {self.root}"
    ...
    def compute_subclasses(self):
        ...
        tree = inspect.getclasstree(
            list(self.name2class.values()), unique=True
        )
...
def get_args():
    ...
    parser.add_argument(
        "--spec", type=str, required=True, help="TOML file to process"
    )
